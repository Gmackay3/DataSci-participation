...
# Order that you give them in is the order that they will be given/shown
```
...
# this works but it doesn't give you any idea what the variables are without actually running the code and can cause issues if you change things around before this line of code in a later version. (making changes after coming back to this code later)
```

Extra example:
```{r}
psych::bfi %>% as_tibble()
psych::bfi %>% as_tibble() %>% select(A1:A5)
```
...
Select() practice using mtcars
```{r}
mtcars

select(mtcars, cyl, hp, mpg)

select(mtcars, -wt)
select(mtcars, -(wt:gear))

select(mtcars, MPG = mpg)
select(mtcars, Cylnd = cyl, everything())
...
```{r}
gapminder %>%
  filter(pop > 100000000) %>% #or you can do in this line filter (pop > 100000000 & continent == "Asia") 
...
gapminder %>%
  filter(pop > 100000000) %>% 
  filter(country == "Brazil" | country == "China") # could use code: country %in% c("Brazil", "China")
...
gapminder %>%
  mutate(grossGDP = gdpPercap * pop,
         gdpBill = round(grossGDP / 1000000000, digits = 2)) #rounding each to 2 decimals

#or 
gapminder %>%
  mutate(grossGDP = gdpPercap * pop,
         gdpBill = (grossGDP / 1000000000) %>% round(2)) # %>% 
  select(-pop, -gdpPercap, -grossGDP) #this line is going to remove the other variables that you do not want or need in your data.
```
...
gapminder %>%
  transmute(country, continent, lifeExp, 
            gdpBill = (gdpPercap * pop / 1000000000) %>% round(2))
```
...
Replace Bangladesh life expectancy with NA using NA_real_.
```{r}
gapminder %>%
