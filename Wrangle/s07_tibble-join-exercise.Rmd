...
```{r}
setdiff(y, z) #this is keeping the values in y that are NOT also in z
```
...
#to see all of the datasets in the package you click on the name menu in the environment tab
```

## Joining all to one dataset
These have to be done step by step and matching up all of the things in common between the groups to help 
```{r}
flights %>% 
  left_join(airlines, by = "carrier") %>% 
  left_join(planes, by = c("tailnum",  "year")) %>% 
  left_join(airports, by = "name") %>%  #this may not be fully correct so check answer sheet for this
  left_join(weather, by = c("year", "origin", "month", "day", "hour", "time_hour"))
```
