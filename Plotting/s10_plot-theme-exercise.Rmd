...
library(readxl)
library(tidyverse)
library(readr) #this is needed in order to read in the csv file
library(svglite)
```
...
```{r}
dir.create(here::here("Plotting", "output", "figures"), recursive = TRUE)
hp_wt_graph <- ggplot(mtcars, aes(hp, wt)) + 
    geom_point()
ggsave(here::here("Plotting", "output", "figures", "hp_wt_graph.png"), hp_wt_graph)
```
...
p_scales +
    ...
    scale_colour_continuous(
        trans  = "log10", 
        breaks = 10^(1:10) #I took this from the next questions. It still doesn't look any different though.
        #I couldn't get this part of it to work on my own. I haven't figured out the function that is supposed to work for it. 
    ) +
    scale_y_continuous(breaks= seq(from = 20, to = 90, by = 10))
```
...
p_scales +
    ...
    scale_colour_continuous(
        trans  = "log10", 
        breaks = 10^(1:10),
        labels = scales::comma_format() # this chanes the values in the key to be in comma formatting
    ) +
