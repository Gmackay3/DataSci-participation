...
gapminder %>% #take gapminder then, 
  filter(year == 2007) %>% #filter it by year 2007 then,
  mutate(continent = fct_infreq(continent)) %>%  #mutate it into a frequency (maybe this?) then,
...
p <- ggplot(mauna, aes(month, conc)) +
  ...
  theme_bw()
p +
  geom_point(colour = "green", alpha = 0.25)
# it just overlapped the line so it may be the same to do geom_point instead of geom_line
```
...
# alternateive is moving the x and y variables into an aes in the first line. Both give you the same thing
```
