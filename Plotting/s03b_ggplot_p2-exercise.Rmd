...
Code to fix the x-axis labels from [here](https://stackoverflow.com/questions/1828742/rotating-axis-labels-in-r)
...
gapminder %>% 
  ...
  ggridges::geom_density_ridges(aes(scale = 2))
```
...
Notes: I was able to get the proportions on the y and the variables discrete but they removed all of the x-labels and I cannot get them to return

```{r, fig.width = 5, fig.height = 2}
mtcars %>% 
  mutate(transmission = if_else(am == 0, "automatic", "manual")) %>%
  ggplot(aes(cyl)) +
  geom_bar(aes(color = transmission, y = ..prop.., fill = transmission)) +
  scale_x_discrete(aes("cyl", breaks = waiver(), labels = scales::label_number(c("4", "6", "8")), limits = )) 
  #xlab(c("4", "6", "8"))
